import { Component, HostListener, OnInit, ViewChild, ElementRef, inject, AfterViewInit } from '@angular/core';
...
@Component({
  selector: 'app-nav',
  imports: [CommonModule, FormsModule, MatTabsModule, MatSlideToggleModule, RouterLink, AsyncPipe, RouterLinkActive],
...
export class Nav implements OnInit, AfterViewInit {
  ...
  // ViewChild references for both desktop and mobile toggles
  @ViewChild('darkModeSwitch', { read: ElementRef }) desktopToggle: ElementRef | undefined;
  @ViewChild('darkModeSwitchMobile', { read: ElementRef }) mobileToggle: ElementRef | undefined;
  ...
  ngOnInit() {
    this.isHandset$.subscribe(isHandset => {
      ...
      if (!this.isDesktop) {
        // Close menu on mobile when switching to mobile view
        this.closeMenu();
      }
    });
  ...
  ngAfterViewInit() {
    // Handle desktop toggle icons
    if (this.desktopToggle) {
      this.updateToggleIcons(this.desktopToggle);
    }
    
    // Handle mobile toggle icons
    if (this.mobileToggle) {
      this.updateToggleIcons(this.mobileToggle);
    }
  }

  /**
   * Updates the custom sun/moon icons for a mat-slide-toggle element
   */
  private updateToggleIcons(element: ElementRef): void {
    try {
      const onIcon = element.nativeElement.querySelector('.mdc-switch__icon--on');
      const offIcon = element.nativeElement.querySelector('.mdc-switch__icon--off');
      
      if (onIcon?.firstChild) {
        onIcon.firstChild.setAttribute('d', this.moon);
      }
      
      if (offIcon?.firstChild) {
        offIcon.firstChild.setAttribute('d', this.sun);
      }
    } catch (error) {
      console.warn('Could not update toggle icons:', error);
    }
  }

  /**
   * Toggles between dark and light mode & applies theme
   */
  public toggleDarkMode(): void {
    ...
    // Update icons for both toggles after theme change
    setTimeout(() => {
      if (this.desktopToggle) {
        this.updateToggleIcons(this.desktopToggle);
      }
